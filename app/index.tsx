...
import { interfaceGen } from '@/@types/interfaceGen';
...
export default function Index() {
    ...
    const [selectedGens, setSelectedGens] = useState<interfaceGen[]>([]);
    const generations: interfaceGen[] = [
        ...
        {gen: 'G9', firstId: 909, lastId: 1025},
    ]
    ...
    const fetchAllPokemon = async () => {
        ...
        try {
            ...
            for (let i = start; i <= end && i <= 1025; i++) {
                requests.push(api.get(`pokemon/${i}`));
            }

            const responses = await Promise.all(requests);
            const newPokemons = responses.map(res => res.data);

            setVisiblePokemon(prev => [...prev, ...newPokemons]);
  ...
  const fetchSelected = async () => {
    ...
    try {
        ...
        const responses = await Promise.all(requests);
        const pokemons = responses.map((pokemon) => pokemon.data)
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => <ShowPokemon pokemon={item} />}
                numColumns={3}
